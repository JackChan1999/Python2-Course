...
| rename()    | 重命名                    |
| remove()    | 删除文件                   |
| uname()     | 获取详细的系统信息，在Windows上不提供 |
| os.name     | 获取操作系统的名称              |
| os.environ  | 获取系统的环境变量              |
| os.getenv() | 获取系统的某个环境变量            |
| listdir()   | 获取⽬录列表                 |
| mkdir()     | 创建文件夹                  |
| rmdir()     | 删除文件夹                  |
| chdir()     | 改变当前目录                 |
...
| dump()  | 直接把对象序列化后写入一个file-likeObject    |
| loads() | 反序列化                            |
| load(f) | 从一个 file-like Object 中直接反序列化出对象 |
...
| 方法声明    | 功能描述                           |
| :------ | :----------------------------- |
| dumps() | 把Python对象变成一个JSON              |
| dump()  | 把转换成的JSON写入一个 file-like Object |
| loads() | 把json反序列化为对象                   |
